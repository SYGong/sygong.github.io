...
IEEE 802.11 定义了`RSSI` (Received Signal Strength Indicator)，范围从`0`到`RSSI_Max`。`RSSI_Max`的值由厂商自己定义，`RSSI`每一个值对应的 $$\mathit{RSS}$$ 也由各家厂商自己定义[[1]][[2]]。[[2]]中还有Atheros等不同厂商的 $$\mathit{RSSI}$$ 计算方式。
